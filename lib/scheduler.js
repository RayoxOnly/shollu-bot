...
export async function startScheduler() {
  ...
  // Always stop the daily refresh job before potentially returning early,
  // so disabling the bot truly stops all scheduled work.
  if (global._schedulerDailyRefresh) {
    global._schedulerDailyRefresh.stop();
    global._schedulerDailyRefresh = null;
  }
  ...
  // Schedule a daily refresh shortly after midnight to pick up new prayer times
  global._schedulerDailyRefresh = cron.schedule('1 0 * * *', async () => {
    // Guard against concurrent restarts (e.g., if triggered twice rapidly)
    if (global._schedulerRefreshing) return;
    global._schedulerRefreshing = true;
    try {
      console.log('[SCHEDULER] 🔄 Memperbarui jadwal harian...');
      invalidatePrayerCache();
      await startScheduler();
    } finally {
      global._schedulerRefreshing = false;
    }
  }, { timezone });
}
