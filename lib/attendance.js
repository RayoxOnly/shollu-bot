 ...
 */
export async function submitOne(token, qrCode, eventId, mesinId, type = 'sholat_wajib') {
...
export async function submitAll(prayer = 'subuh') {
  ...
  const absenType = prayer === 'tarawih' ? 'sholat_tarawih' : 'sholat_wajib';
  const results = [];
  ...
  for (let i = 0; i < qrCodes.length; i++) {
    const qr = qrCodes[i];
    try {
      const response = await submitOne(token, qr.qr_code, eventId, mesinId, absenType);
      ...
      if (err.response) {
        ...
        if ((status === 401 || status === 403) && !hasRetried) {
          ...
          try {
            ...
            // Retry this QR code
            const retryRes = await submitOne(token, qr.qr_code, eventId, mesinId, absenType);
