...
const ABSEN_TARAWIH_URL = 'https://api.shollu.com/api/v1/absent-qr-tarawih';
 ...
 * Tarawih uses a separate endpoint (/absent-qr-tarawih).
 */
export async function submitOne(token, qrCode, eventId, mesinId, isTarawih = false) {
  const url = isTarawih ? ABSEN_TARAWIH_URL : ABSEN_URL;
...
export async function submitAll(prayer = 'subuh') {
  ...
  const isTarawih = prayer === 'tarawih';
  const results = [];
  ...
  for (let i = 0; i < qrCodes.length; i++) {
    const qr = qrCodes[i];
    try {
      const response = await submitOne(token, qr.qr_code, eventId, mesinId, isTarawih);
      ...
      if (err.response) {
        ...
        if ((status === 401 || status === 403) && !hasRetried) {
          ...
          try {
            ...
            // Retry this QR code
            const retryRes = await submitOne(token, qr.qr_code, eventId, mesinId, isTarawih);
