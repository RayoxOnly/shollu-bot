...
const API_KEY = process.env.SHOLLU_API_KEY || (process.env.NODE_ENV !== 'production' ? 'shollusemakindidepan' : null);

/**
 * Submit attendance for a single QR code.
 */
export async function submitOne(token, qrCode, eventId, mesinId) {
...
export async function submitAll(prayer = 'subuh') {
  if (!API_KEY) {
    const msg = 'SHOLLU_API_KEY environment variable is required in production';
    console.error(`[ABSEN] ❌ ${msg}`);
    addLog('-', '-', 'error', msg, prayer);
