...
export const PRAYERS = ['subuh', 'dzuhur', 'ashar', 'maghrib', 'isya', 'tarawih'];

// ---- Cached prepared statements ----
// better-sqlite3 compiles statements once; reuse them for efficiency.
...
const stmtGetQrCodes = db.prepare('SELECT * FROM qr_codes ORDER BY created_at DESC');
const stmtGetEnabledQrCodes = db.prepare('SELECT * FROM qr_codes WHERE enabled = 1 ORDER BY id ASC');
const stmtAddQrCode = db.prepare('INSERT INTO qr_codes (name, qr_code) VALUES (?, ?)');
const stmtRemoveQrCode = db.prepare('DELETE FROM qr_codes WHERE id = ?');
const stmtToggleQrCode = db.prepare(
  'UPDATE qr_codes SET enabled = CASE WHEN enabled = 1 THEN 0 ELSE 1 END WHERE id = ?'
);
const stmtAddLog = db.prepare(
  'INSERT INTO logs (qr_code, name, prayer, status, message) VALUES (?, ?, ?, ?, ?)'
);
const stmtGetLogs = db.prepare('SELECT * FROM logs ORDER BY id DESC LIMIT ?');
const stmtClearLogs = db.prepare('DELETE FROM logs');
const stmtMarkAttendance = db.prepare(
...
const stmtUnmarkAttendance = db.prepare(
  `INSERT INTO attendance (date, prayer, status, marked_at)
   VALUES (?, ?, 'pending', NULL)
   ON CONFLICT(date, prayer) DO UPDATE SET status = 'pending', marked_at = NULL`
);
const stmtGetAttendanceByDate = db.prepare('SELECT * FROM attendance WHERE date = ?');
const stmtGetAttendanceRange = db.prepare(
  'SELECT * FROM attendance WHERE date >= ? AND date <= ? ORDER BY date ASC'
);
const stmtPerPrayerCount = db.prepare(
  `SELECT COUNT(*) as count FROM attendance
   WHERE date >= ? AND date <= ? AND prayer = ? AND status = 'completed'`
);
const stmtTotalCount = db.prepare(
  `SELECT COUNT(*) as count FROM attendance
...
export function setSetting(key, value) {
  stmtSetSetting.run(key, value, value);
}

export function setSettings(obj) {
  const tx = db.transaction(() => {
    for (const [key, value] of Object.entries(obj)) {
      stmtSetSetting.run(key, String(value), String(value));
    }
  });
  tx();
}
...
export function getQrCodes() {
  return stmtGetQrCodes.all();
}

export function getEnabledQrCodes() {
  return stmtGetEnabledQrCodes.all();
}

export function addQrCode(name, qrCode) {
  return stmtAddQrCode.run(name, qrCode);
}

export function removeQrCode(id) {
  return stmtRemoveQrCode.run(id);
}

export function toggleQrCode(id) {
  return stmtToggleQrCode.run(id);
}

// ---- Log helpers ----

export function addLog(qrCode, name, status, message, prayer = 'subuh') {
  return stmtAddLog.run(qrCode, name, prayer, status, message || '');
}

export function getLogs(limit = 100) {
  return stmtGetLogs.all(limit);
}

export function clearLogs() {
  return stmtClearLogs.run();
}

// ---- Attendance helpers ----

export function markAttendance(date, prayer, status = 'completed') {
  return stmtMarkAttendance.run(date, prayer, status, status);
}

export function unmarkAttendance(date, prayer) {
  return stmtUnmarkAttendance.run(date, prayer);
}

export function getAttendanceByDate(date) {
  const rows = stmtGetAttendanceByDate.all(date);
  const result = {};
...
export function getAttendanceRange(startDate, endDate) {
  return stmtGetAttendanceRange.all(startDate, endDate);
}
...
export function getCompletionStats(days = 30) {
  ...
  const total = stmtTotalCount.get(startStr, endStr);
  ...
  const perPrayer = {};
  for (const p of PRAYERS) {
    const pCount = stmtPerPrayerCount.get(startStr, endStr, p);
    perPrayer[p] = {
