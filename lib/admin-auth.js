 ...
 *
 * When ADMIN_TOKEN is not set, all requests are allowed (for local development).
 */
export function isAuthorized(req) {
  const expectedToken = process.env.ADMIN_TOKEN;
  if (!expectedToken) {
    // In production, require ADMIN_TOKEN to prevent accidental open deployments.
    if (process.env.NODE_ENV === 'production') {
      return false;
    }
    // No token configured in development – allow access for local use.
    return true;
  }
